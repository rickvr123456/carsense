...
class GeminiService {
  ...
  Future<Map<String, Dtc>> describeDtcs(List<String> dtcCodes) async {
    if (dtcCodes.isEmpty) {
      debugPrint('[Gemini] Nessun codice da descrivere');
      return {};
    }
...
''';
    ...
    try {
      debugPrint('[Gemini] Invio prompt: ${prompt.substring(0, prompt.length.clamp(0, 300))}');
      final contents = [Content.text(prompt)];
      final response = await model.generateContent(contents);

      final text = response.text;
      debugPrint('[Gemini] Risposta: ${text?.substring(0, text.length.clamp(0, 400)) ?? "<vuoto>"}');

      return _parseJsonResponse(text ?? '');
    } catch (e, st) {
      debugPrint('[Gemini][ERRORE] $e');
      debugPrint('$st');
      return {};
    }
  }
  ...
  Map<String, Dtc> _parseJsonResponse(String text) {
    try {
      ...
      if (decoded is Map<String, dynamic>) {
        ...
        debugPrint('[Gemini] Parsed ${result.length} DTC dettagliati.');
        return result;
      }
    } catch (e) {
      debugPrint('[Gemini] Errore parsing JSON: $e');
      ...
      if (start != -1 && end != -1 && end > start) {
        ...
        try {
          ...
          if (decoded is Map<String, dynamic>) {
            ...
            debugPrint('[Gemini] Parsed (fallback) ${result.length} DTC dettagliati.');
            return result;
          }
        } catch (e2) {
          debugPrint('[Gemini] Fallback parse fallito: $e2');
        }
      }
      return {};
    }
